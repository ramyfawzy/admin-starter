...
public class CarService implements Serializable {
    ...
    private List<Predicate<Car>> configFilter(Filter<Car> filter) {
        ...
        if (filter.hasFilterParam("id")) {
            Predicate<Car> idPredicate = (Car c) -> c.getId().equals(filter.getFilterParam("id"));
            predicates.add(idPredicate);
        }

        if (filter.hasFilterParam("minPrice") && filter.hasFilterParam("maxPrice")) {
            Predicate<Car> minMaxPricePredicate = (Car c) -> c.getPrice()
                    >= Double.valueOf((String) filter.getFilterParam("minPrice").getFilterValue()) && c.getPrice()
                    <= Double.valueOf((String) filter.getFilterParam("maxPrice").getFilterValue());
            predicates.add(minMaxPricePredicate);
        } else if (filter.hasFilterParam("minPrice")) {
            Predicate<Car> minPricePredicate = (Car c) -> c.getPrice()
                    >= Double.valueOf((String) filter.getFilterParam("minPrice").getFilterValue());
            predicates.add(minPricePredicate);
        } else if (filter.hasFilterParam("maxPrice")) {
